...
public class ButtonControl2 : MonoBehaviour
{
	...
	byte[] results = handshake.downloadHandler.data;
	Debug.Log(handshake.downloadHandler.data);
	...
	if (handshake.isNetworkError || handshake.isHttpError)
            ...
            var json = JsonConvert.DeserializeObject<PreprocessResponse>(System.Text.Encoding.Default.GetString(results));
